...
export const command: PojavCommand = {
  ...
  async listener(interaction, _) {
    ...
    if (!interaction.member.permissions.has("MANAGE_MESSAGES")) return await interaction.editReply("You do not have permission to run this command.");
    if (interaction.options.getBoolean("showexecutorname") && !interaction.member.permissions.has("ADMINISTRATOR")) return await interaction.editReply("You do not have permission to not show the executor name.");
    if (!channel?.isText())  return await interaction.editReply("Specified channel is not a text channel.");
    await channel?.send({ content: message });
    return await interaction.editReply("Successfully sent message.");
  },
};